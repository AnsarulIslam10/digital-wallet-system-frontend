...
export default function Transactions() {
  ...
  const [filterType, setFilterType] = useState<"all"|"add"|"withdraw"|"send"|"receive"|"fee"|"cash-in"|"cash-out">("all");
  ...
  const { data, isLoading, isError } = useGetMyTransactionsQuery(queryArgs, { refetchOnMountOrArgChange: true });
  ...
  const getTransactionBadge = (type: string) => {
    ...
    return <Badge variant="outline" className={badge[type] || badge.default}>{type.replace("-", " ")}</Badge>;
  };

  const formatAmount = (amount: number, type: string) => {
    const neg = ["withdraw","send","fee","cash-out"].includes(type);
    return <span className={`font-semibold ${neg ? "text-red-600" : "text-green-600"}`}>{neg ? "-" : "+"} {amount} BDT</span>;
  };
          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
            ...
            <div className="flex gap-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  {["all","add","withdraw","send","receive","fee","cash-in","cash-out"].map(t=>(
                    <DropdownMenuItem key={t} onClick={()=>setFilterType(t as any)}>
